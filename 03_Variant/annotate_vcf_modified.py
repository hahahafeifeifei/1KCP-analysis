...
#Decompose multiallelic variant site to biallelic variant alleles
#python3 annotate_vcf_modified.py -vcf input.vcf -gfa input.gfa -o output.vcf
...
def parse_gfa(filename, ref_prefix = "GRCh38", segments = None):
        """
        ...
        with open(filename, 'r') as gfa_file:
                for line in gfa_file:
                        if line[0] in ['S']:
                                ...
                                node_to_data[fields[1]] = ([], fields[2])
                        if line[0] in ['W']:
                                fields = line.split()
                                if ref_prefix in fields[1]:
                                        start_pos = int(fields[4])
                                        ref_pos = start_pos
                                        path_info = fields[6]
                                        path_node = re.split('<|>', path_info)[1:]
                                        for node in path_node:
                                                node_to_data[node][0].append(ref_pos)
                                                ref_pos += len(node_to_data[node][1])
        return node_to_data
...
def detect_variants(ref_traversal, alt_traversal, gfa):
        """
        ...
        for i,node in enumerate(ref_traversal):
                ...
                if node in node_to_index:
                        ...
                        if (abs(alt_index - prev_alt_index) > 1) or (abs(i-prev_ref_index) > 1):
                                ...
                                ref_allele = ref_traversal[prev_ref_index : i+1]
                                if len(allele) > 1:
                                        allele_pairs.append([ref_allele, allele, pre_pos])
...
def get_ref_position(node, origin_pos, gfa, add_flank=True):
        """
        Look up reference position of a segment in graph.
        """
        assert node.id in gfa
        pos_dist = [abs(pos - origin_pos) for pos in gfa[node.id][0]]
        new_pos = gfa[node.id][0][pos_dist.index(min(pos_dist))]
        position = new_pos + len(gfa[node.id][1])
        if add_flank:
                return str(position)
        else:
...
def decompose(line, gfa):
	...
	alt_len = [len(alt) for alt in fields[4].split(",")]
	parent_mnv_origin = (ref_len == max(alt_len)) and (ref_len == min(alt_len))
	...
	if nr_alleles == 1:
		...
		return updated_line, [updated_line]
                ...
                for i,a in enumerate(allele_traversals[1:]):
                        ...
                        for allele_pair in detect_variants(ref_traversal, alt_traversal, gfa):
                                ref_allele = allele_pair[0]
                                allele = allele_pair[1]
                                #print(str(i), end = " ")
                                #print([i.id for i in allele], end = " ")
                                #print([i.id for i in ref_allele])
                                ref_pos = int(fields[1]) + allele_pair[2]
				# ref_pos normalize for parent mnv (-1) or son mnv(+1)
                                son_mnv_origin = len(construct_allele_string(ref_allele, gfa, False)) == len(construct_allele_string(allele, gfa, False))
                                if parent_mnv_origin:
                                        ref_pos -= 1
                                if son_mnv_origin:
                                        ref_pos += 1
                                add_flank = not son_mnv_origin
...
if __name__== "__main__":
        ...
        with open(args.o + '.vcf', 'w') as out_multi, open(args.o + '.biallelic.vcf', 'w') as out_bi:
